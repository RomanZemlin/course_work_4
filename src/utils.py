...
def create_instances_from_hh(database) -> None:
    """
    Создаём экземпляры класса Vacancy с headhunter
    И проверяем, чтобы зарплата была в рублях, иначе игнорируем вакансию
...
def create_instances_from_sj(database, query) -> None:
    """
    Создаём экземпляры класса Vacancy с superjob
    И проверяем, чтобы зарплата была в рублях, иначе игнорируем вакансию
